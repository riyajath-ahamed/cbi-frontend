
const CardWait = () => {
  return (
    <div className="mx-auto mt-12 max-w-2xl sm:mt-16 lg:mt-16 lg:max-w-6xl">
      <dl className="grid max-w-2xl grid-cols-1 gap-x-8 gap-y-8 lg:max-w-none lg:grid-cols-3 lg:gap-y-24">
    <div class="flex flex-col bg-neutral-300 w-72 h-96 animate-pulse rounded-xl p-4 gap-4">
      <div class="bg-neutral-400/50 w-full h-32 animate-pulse rounded-md"></div>
      <div class="flex flex-col gap-2">
        <div class="bg-neutral-400/50 w-full h-4 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-4/5 h-8 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-full h-4 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-2/4 h-20 animate-pulse rounded-md"></div>
      </div>
    </div>
    <div class="flex flex-col bg-neutral-300 w-72 h-96 animate-pulse rounded-xl p-4 gap-4">
      <div class="bg-neutral-400/50 w-full h-32 animate-pulse rounded-md"></div>
      <div class="flex flex-col gap-2">
        <div class="bg-neutral-400/50 w-full h-4 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-4/5 h-8 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-full h-4 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-2/4 h-20 animate-pulse rounded-md"></div>
      </div>
    </div>
    <div class="flex flex-col bg-neutral-300 w-72 h-96 animate-pulse rounded-xl p-4 gap-4">
      <div class="bg-neutral-400/50 w-full h-32 animate-pulse rounded-md"></div>
      <div class="flex flex-col gap-2">
        <div class="bg-neutral-400/50 w-full h-4 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-4/5 h-8 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-full h-4 animate-pulse rounded-md"></div>
        <div class="bg-neutral-400/50 w-2/4 h-20 animate-pulse rounded-md"></div>
      </div>
    </div>
    </dl>
    
    </div>
  );
};

export default CardWait;
